...
class PurchaseBillSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = PurchaseBill
        fields = ['id', 'vendor', 'vendor_name', 'items', 'date']
        read_only_fields = ['id', 'date']
    ...
    def create(self, validated_data):
        ...
        for item_data in items_data:
            ...
            # ── Batch logic ──────────────────────────────────────────────────
            # Normalize MRP to 2 decimal places for comparison
            mrp_decimal = Decimal(str(mrp)).quantize(Decimal('0.01'))

            # Find existing batch with exactly this MRP
            existing_batch = None
            ...
            # Update product totals — selling_price = latest MRP, unit = latest
...
class SaleBillSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for item_data in items_data:
            ...
            # Extract batch_id from item_data (it's write_only so won't be in validated normally)
            ...
            if batch:
                ...
                # No batch specified — deduct FIFO (cheapest/oldest first)
                remaining = qty
...
class PurchaseReturnSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'product', 'product_name', 'product_barcode',
                  'quantity', 'purchase_price', 'tax', 'item_cost', 'reason', 'date']
